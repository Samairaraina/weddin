...
async def fetch_unsplash_images(function_type: str, per_page: int = 12) -> list[dict]:
    ...
    return await fetch_unsplash_images_page(function_type=function_type, per_page=per_page, page=1, query=query, access_key=access_key)


async def fetch_unsplash_images_page(function_type: str, per_page: int = 12, page: int = 1, query: str | None = None, access_key: str | None = None) -> list[dict]:
    access_key = access_key or os.getenv("UNSPLASH_ACCESS_KEY")
    query = query or f"indian wedding {function_type} decor"
    if access_key:
